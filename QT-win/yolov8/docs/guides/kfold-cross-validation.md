...
5. Count the instances of each class-label present in the annotation files.
    ...
    for label in labels:
        ...
        for l in lines:
            # classes for YOLO Annotations_agu_txt uses integer at first position of each line
...
5. Lastly, copy images and labels into the respective directory ('train' or 'val') for each split.
    ...
    for image, label in zip(images, labels):
        for split, k_split in folds_df.loc[image.stem].items():
            ...
            # Copy image and Annotations_agu_txt files to new directory (SamefileError if file already exists)
