...
def img2label_paths(img_paths):
    """Define Annotations_agu_txt paths as a function of image paths."""
...
def verify_image(args):
    ...
    try:
        ...
        nc = 1
        msg = f'{prefix}WARNING ⚠️ {im_file}: ignoring corrupt image/Annotations_agu_txt: {e}'
    return (im_file, cls), nf, nc, msg


def verify_image_label(args):
    """Verify one image-Annotations_agu_txt pair."""
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            nf = 1  # Annotations_agu_txt found
            ...
            if nl:
                ...
                assert lb.min() >= 0, f'negative Annotations_agu_txt values {lb[lb < 0]}'

                # All labels
                max_cls = lb[:, 0].max()  # max Annotations_agu_txt count
                ...
                ne = 1  # Annotations_agu_txt empty
                lb = np.zeros((0, (5 + nkpt * ndim) if keypoint else 5), dtype=np.float32)
        else:
            nm = 1  # Annotations_agu_txt missing
...
def autosplit(path=DATASETS_DIR / 'coco8/images', weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    for i, img in TQDM(zip(indices, files), total=n):
        if not annotated_only or Path(img2label_paths([str(img)])[0]).exists():  # check Annotations_agu_txt
