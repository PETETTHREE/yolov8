...
class Open_Camera(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def taking_pictures(self):
        if self.cap.isOpened():
            ...
            self.showImage.save('.\\Picture\\1.jpg')
        else:
    ...
    # 导入图片
    def loadphoto(self):
        initial_path = ".\Picture"  # 将"指定的路径"替换为你想要的路径
        fname, _ = QFileDialog.getOpenFileName(self, '选择图片', initial_path, 'Image files(*.jpg *.gif *.png *.bmp)')
        if fname:
